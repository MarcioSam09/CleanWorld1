...
async function createUsuario(nome,cpf,endereco,telefone,email,tipoCadastro){
    const connection = await mysql.createConnection(databaseConfig);

    const insertUsuario = "insert into usuario(nome,cpf,endereco,telefone,email,tipocadastro) values (?,?,?,?,?,?)";

    await connection.query(insertUsuario,[nome,cpf,endereco,telefone,email,tipoCadastro])
