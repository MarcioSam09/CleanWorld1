...
async function updateFreteiro(req, res){
    try{
        const {id} = req.params;
        const {distancia, tipoColeta, peso, nomeCarro, modeloCarro, placaCarro} = req.body;

        await freteiroService.updateFreteiro(distancia, tipoColeta, peso, nomeCarro, modeloCarro, placaCarro, id);
